...
local MAJOR_VERSION = "TagCompiler-2.0"
local MINOR_VERSION = tonumber(string.match("$Revision: 2 $", "%d+")) or 0

local TagCompiler = LibStub:NewLibrary(MAJOR_VERSION, MINOR_VERSION)
if not TagCompiler then return end
...
TagCompiler.CommonTagData.ColorIf = {
	method = function(data, value, condition, r, g, b) return value and condition and ("|cff%02x%02x%02x%s|r"):format(r*255, g*255, b*255, value) or value end,
	noGuaranteeChange = true,
	parameterTypes = { "raw", "number", "number", "number" }
}

TagCompiler.CommonTagData.Contains = {
	method = function(data, value, s, c) return value and s:find(c) end,
...
function TagCompiler:Compile(tag, tagData, resultType)
	assert(type(tag) == "string", "Bad argument #2 to :Compile (string expected)")
	assert(type(tagData) == "table", "Bad argument #3 to :Compile (table expected)")
	assert(resultType == "string" or resultType == "number" or resultType == "raw", "Bad argument #4 to :Compile (resultType must be string, number, or raw)")
...
function TagCompiler:HighlightSyntax(tag, tagData, resultType)
	assert(type(tag) == "string", "Bad argument #2 to :HighlightSyntax (string expected)")
	assert(type(tagData) == "table", "Bad argument #3 to :HighlightSyntax (table expected)")
	assert(resultType == "string" or resultType == "number" or resultType == "raw", "Bad argument #4 to :HighlightSyntax (resultType must be string, number, or raw)")
...
function TagCompiler:FixCasing(tag, tagData, resultType)
	assert(type(tag) == "string", "Bad argument #2 to :FixCasing (string expected)")
	assert(type(tagData) == "table", "Bad argument #3 to :FixCasing (table expected)")
	assert(resultType == "string" or resultType == "number" or resultType == "raw", "Bad argument #4 to :FixCasing (resultType must be string, number, or raw)")
...
function TagCompiler:IsValid(tag, tagData, resultType)
	assert(type(tag) == "string", "Bad argument #2 to :IsValid (string expected)")
	assert(type(tagData) == "table", "Bad argument #3 to :IsValid (table expected)")
	assert(resultType == "string" or resultType == "number" or resultType == "raw", "Bad argument #4 to :IsValid (resultType must be string, number, or raw)")

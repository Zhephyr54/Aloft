...
local MINOR_VERSION = 90000 + tonumber(("$Revision: 70 $"):match("%d+")) or 0
...
{
	method = function(data, value) return (value and value ~= "" and ("<"..value..">")) or "" end,
...
{
	method = function(data, value, r, g, b) return (value and ("|cff%02x%02x%02x%s|r"):format(r*255, g*255, b*255, value)) or "" end,
	noGuaranteeChange = true,
...
{
	method = function(data, value, format) return (value and format:format(value)) or "" end,
	noGuaranteeChange = true,
...
{
	method = function(data, value, prefix) return (value and value ~= "" and prefix..value) or "" end,
	noGuaranteeChange = true,
	parameterTypes = { "string" }
}
...
{
	method = function(data, value, prefix, suffix) return (value and value ~= "" and prefix..value..suffix) or "" end,
	noGuaranteeChange = true,
	parameterTypes = { "string", "string" }
}
...
{
	method = function(data, value, r, g, b) return (value and ("|cff%02x%02x%02x%s|r"):format(r, g, b, value)) or "" end,
	parameterTypes = { "number", "number", "number" }
}
...
{
	method = function(data, value, suffix) return (value and value ~= "" and value..suffix) or "" end,
	noGuaranteeChange = true,
	parameterTypes = { "string" }
}
...
local function ParsePrimary(functionString)
	...
	elseif (byte >= 65 and byte <= 90) or (byte >= 97 and byte <= 122) then	-- byte >= 'A' && byte <= 'Z' || byte >= 'a' && byte <= 'z'
		...
		elseif tagData.method then
			...
			if not dataTable.error then
				if #parameterStringList > 0 then
					result = ('(tagMethod%s(data, %s, %s) or "")'):format(identifier, functionString or "true", table.concat(parameterStringList, ", "))
					parameterStringList = del(parameterStringList)
				else
					result = ('(tagMethod%s(data, %s) or "")'):format(identifier, functionString or "true")
				end
			end
		end
...
function TagCompiler:Compile(tag, tagData, resultType)
	...
	if dataTable.error then
		...
		local method = loadstring(functionString, dataTable.tag)
		-- ChatFrame7:AddMessage("TAG: " .. tostring(dataTable.tag) .. "/" .. tostring(functionString))

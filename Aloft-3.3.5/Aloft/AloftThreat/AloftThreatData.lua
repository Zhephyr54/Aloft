...
AloftThreatData.db = Aloft:AcquireDBNamespace("threatData")
Aloft:RegisterDefaults("threatData", "profile", {
...
function AloftThreatData:OnEnable()
	Aloft:DebugRegister("ThreatData")
...
function AloftThreatData:EnableDataSource()
	...
	-- TODO: set threat bar/text profile.enable flag to false
	...
	local enable = ((Aloft:HasModule("ThreatBar") and Aloft:GetModule("ThreatBar").db.profile.enable) or
					    (Aloft:HasModule("ThreatText") and Aloft:GetModule("ThreatText").db.profile.enable)) and
						(profile and (profile.AOEEnable or profile.groupEnable))
	if enable then
		-- for update of target-of-target data on hostile nameplates
		-- ChatFrame7:AddMessage("AloftThreatData:EnableDataSource(): schedule")
		self:ScheduleRepeatingEvent("AloftThreatData:ProcessNonTargetThreatList", self.ProcessNonTargetThreatList, profile.interval, self)
	end
...
function AloftThreatData:OnPlayerTargetChanged()
	-- ChatFrame7:AddMessage("AloftThreatData():OnPlayerTargetChanged(): enter")

	-- collect the target GUID if there is one; if not, the cache SHOULD be nil
	targetGUId = nil
...
function AloftThreatData:OnUnitThreatListUpdate(targetId)
...
function AloftThreatData:OnUnitThreatSituationUpdate(unitid)
...
function AloftThreatData:OnUnitidDataChanged(aloftData)
...
function AloftThreatData:OnIsTargetDataChanged(aloftData)
...
function AloftThreatData:OnUnitTargetDataChanged(aloftData, unitid)
...
function AloftThreatData:ProcessThreatUpdate(aloftData, originId, originGUId, targetId, targetGUId, tgtgtGUId, threat)
	...
	local isTanking, state, scaledPercent, rawPercent, threatValue = UnitDetailedThreatSituation(originId, targetId)
	...
	if threatValue then
		...
		self:SetThreatData(aloftData, threat, maxThreat, threatMulti, threatType)
	end
...
function AloftThreatData:ProcessNonTargetThreatList()
	...
	for aloftData in Aloft:IterateVisibleNameplates() do
		if aloftData ~= targetNameplate and (not aloftData.isTarget or aloftData:IsTarget()) then -- trust threat events to deliver threat for the player's current target
			...
			if profile.groupEnable and originId and targetGUId and (aloftData.type == "friendlyPlayer" or aloftData.type == "friendlyNPC") then -- friendly/groupmember threat versus player's active target
			...
			elseif profile.AOEEnable and targetTracking and (aloftData.type == "hostilePlayer" or aloftData.type == "hostile" or aloftData.type == "neutral") then -- hostile/AOE threat versus player, if we have targets tracked
...
function AloftThreatData:SetThreatData(aloftData, threat, maxThreat, threatMulti, threatType)
